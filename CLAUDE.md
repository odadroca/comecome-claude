...
- `pages/guardian/` — dashboard, **manage-users** (child + guardian admin — child **sex/DOB** demographics live here), manage-foods/meals/medications/sleep/logs, settings, exports, database backup/restore; shared `nav.php`. ⚠️ **`manage-children.php` is DEAD CODE** — `index.php` routes both `manage-children` and `manage-users` to `manage-users.php` (backward-compat alias); edit `manage-users.php`, not `manage-children.php`. Every guardian POST handler has a `verifyCsrf()` gate; every guardian `<form method="POST">` has `csrfField()`. Exports split across `export.php` (coordinator) + `export-csv.php` + `export-html.php` (keep the four export surfaces in parity).
